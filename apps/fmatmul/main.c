...
// Verify the matrix
int verify_matrix(double *result, double *gold, size_t R, size_t C, double threshold) {
...
int main() {
  ...
  for (uint64_t s = 4; s <= M; s *= 2) {
    ...
    // Verify the result only for s == M (this is only to keep it simple)
    if (s == M) {
      printf("Verifying result...\n");
      int error = verify_matrix(c, g, s, s, DELTA);
      if (error != 0) {
        printf("Error code %d\n", error);
        printf("c[%d]=%f\n", error, c[error]);
        return error;

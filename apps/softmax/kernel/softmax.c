...
void softmax(const float* i, const float* o, float* buf, uint64_t channels,
                        uint64_t innerSize) {
  ...
  for (size_t outerDim = 0; outerDim < outerSize; outerDim++) {
    ...
    for (size_t cnDim = 1; cnDim < channels; cnDim++) {
      for (size_t i = 0; i < innerSize; i++)
        bufPtr[bufOffset + i] = fmax(bufPtr[bufOffset + i], srcPtr[srcOffset + cnDim * cnStep + i]);
      }
    }
  ...
  for (size_t vl = vsetvl_e32m1(avl); avl > 0; avl -= vl) {
    ...
    */
    ...
    for (uint64_t ch = 0; ch < channels; ++ch) {
      ...
      // Exponentiate
      buf_chunk_v = __exp_2xf32(buf_chunk_v);
      // Store the numerator to memory
      vse32_v_f32m1(__o, buf_chunk_v, vl);
      // Accumulate
      den_chunk_v = vfadd_vv_f32m1(den_chunk_v, buf_chunk_v);
    ...
    */
    ...
    for (uint64_t ch = 0; ch < channels; ++ch) {
      ...
      // Divide
      res_chunk_v = vfdiv_vv_f32_m1(num_chunk_v, den_chunk_v, vl);
